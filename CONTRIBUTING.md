...
Use the [issue templates](https://github.com/pippopapera/nova-display-calibration/issues/new/choose) for bugs, measurement reports, or feature and compatibility requests. General questions can use a blank issue. Please follow the [code of conduct](CODE_OF_CONDUCT.md) in project discussions.

For application bugs, include the exact firmware build, app version, selected profile, brightness, reproduction steps, and relevant status-log text. If you have already restored the original profile and restarted, include whether the issue persisted; this is not a prerequisite for reporting a failure. Avoid posting device identifiers, location data, credentials, or full firmware files.
